...
saveas(figure(8),'Saturation profile, Surfactant floodings, Water flooded residual oil.tif')
...
for i=1:(length(t_OBb1))
    ...
    ER_SFtotal(i)=ER_WFIC+Sw(i_OBb1+i-1)-Scw-(fw_SF(i_OBb1+i-1)-1)/(dfds_SF(i_OBb1+i-1));
end

ER_SF=[ER_SF1,ER_SF5,ER_SF2(1)];
ER_SFtotal1=[ER_SF1+ER_WFIC,ER_SF5+ER_WFIC,ER_SF2(1)+ER_WFIC];
...
saveas(figure(10),'Surfactant EOR.tif')

figure(11) % Polymer & Water Flooding Fractional Flow Curve and Tangent Line, Water Sturation
plot(t_OBf,ER_SFtotal1,'b',t_OBb1,ER_SFtotal,'b','linewidth',2)
axis([0 5 0 1])
xlabel('Dimensionless Time','fontsize',16)
ylabel('Recovery Factor','fontsize',16)

t_OBf_All=[t_OBf,t_OBb1];
EOR_All=[ER_SFtotal1,ER_SFtotal];
EOR1=[t_OBf_All' EOR_All'];

save SFEOR_His_total.mat EOR1

saveas(figure(11),'Surfactant EOR,total.tif')

figure(12)
load SFEOR_His_IC.mat
t_OBf_All_IC=EOR1(:,1);
EOR_All_IC=EOR1(:,2);
plot(t_OBf_All_IC,EOR_All_IC,'b','linewidth',2)
hold on
load SFEOR_His_total.mat
t_OBf_All=EOR1(:,1);
EOR_All=EOR1(:,2);
plot(t_OBf_All,EOR_All,'r','linewidth',2)
hold off
axis([0 5 0 1])
xlabel('Dimensionless Time','fontsize',16)
ylabel('Recovery Factor','fontsize',16)
legend({'Surfactant Flooding, Secondary','Surfactant Flooding, Tertiary'},'location','NorthWest','fontsize',11)
saveas(figure(12),'Comparison Surfactant EOR,Initial Condition.tif')
